...
TEST(StdThread_SimpsonMethodTest, can_integrate_2d_function) {
    ...
    double square = SimpsonMethod::parallel(
        parabola, seg_begin, seg_end, 100, std::thread::hardware_concurrency());
...
TEST(StdThread_SimpsonMethodTest, can_integrate_3d_function) {
    std::vector<double> seg_begin = {0, 0};
    std::vector<double> seg_end = {1, 1};
    double volume = SimpsonMethod::parallel(
        body, seg_begin, seg_end, 100, std::thread::hardware_concurrency());
...
TEST(StdThread_SimpsonMethodTest, can_integrate_super_function) {
    ...
    double integral = SimpsonMethod::parallel(
        super, seg_begin, seg_end, 100, std::thread::hardware_concurrency());
