...
#include "../../../modules/task_2/zoreev_m_trapezoidal_rule/trapezoidal_rule.h"
...
double integrate(const std::function<double(std::vector<double>)>& f,
    ...
    #pragma omp parallel for shared(dimension_divider, from, to, f, steps) reduction(+ : result)
    for (int32_t i = 1; i < blocks_count - 1; i++) {
