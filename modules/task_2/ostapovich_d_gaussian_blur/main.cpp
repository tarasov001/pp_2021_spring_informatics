...
TEST(Omp_Gaussian_blur, GetRandomMatrix_Throws_On_Zero_Args) {
    ASSERT_ANY_THROW(getRandomMatrix(0, 0));
}

TEST(Omp_Gaussian_blur, GetRandomMatrix_Throws_On_Negative_Args) {
    ASSERT_ANY_THROW(getRandomMatrix(-1, -1));
}

TEST(Omp_Gaussian_blur, GetRandomMatrix_No_Throw_On_Valid_Args) {
    ASSERT_NO_THROW(getRandomMatrix(3, 5));
}

TEST(Omp_Gaussian_blur, GetRandomMatrix_Returns_Vector_Of_Valid_Size) {
    auto vec = getRandomMatrix(2, 7);
    ASSERT_EQ(vec.size(), (size_t)14);
}

TEST(Omp_Gaussian_blur, CalculateCore_Returns_Valid_Core) {
    int size = 5;
...
TEST(Omp_Gaussian_blur, Filter_Throws_On_Zero_Size_Matrix) {
    std::vector<uint8_t> mat;
    ASSERT_ANY_THROW(filter(mat, 10));
}

TEST(Omp_Gaussian_blur, Filter_Throws_On_Zero_Or_Negative_Width_Value) {
    auto mat = getRandomMatrix(11, 21);
    ASSERT_ANY_THROW(filter(mat, 0));
    ASSERT_ANY_THROW(filter(mat, -1));
}

TEST(Omp_Gaussian_blur, Filter_Throws_Invalid_Width_Value) {
    auto mat = getRandomMatrix(11, 21);
    ASSERT_ANY_THROW(filter(mat, 10));
}

TEST(Omp_Gaussian_blur, Filter_Throws_On_Invalid_Core_Size) {
    int width = 11;
    auto mat = getRandomMatrix(11, 21);
...
TEST(Omp_Gaussian_blur, Filter_No_Throw_On_Valid_Args) {
    int width = 21;
    int coreSize = 3;
...
TEST(Omp_Gaussian_blur, FilterParallel_Throws_On_Zero_Size_Matrix) {
    std::vector<uint8_t> mat;
    ASSERT_ANY_THROW(filterParallel(mat, 10));
}

TEST(Omp_Gaussian_blur, FilterParallel_Throws_On_Zero_Or_Negative_Width_Value) {
    auto mat = getRandomMatrix(11, 21);
    ASSERT_ANY_THROW(filterParallel(mat, 0));
    ASSERT_ANY_THROW(filterParallel(mat, -1));
}

TEST(Omp_Gaussian_blur, FilterParallel_Throws_Invalid_Width_Value) {
    auto mat = getRandomMatrix(10, 20);
    ASSERT_ANY_THROW(filterParallel(mat, 3));
}

TEST(Omp_Gaussian_blur, FilterParallel_Throws_On_Invalid_Core_Size) {
    auto mat = getRandomMatrix(11, 21);
...
TEST(Omp_Gaussian_blur, FilterParallel_No_Throw_On_Valid_Args) {
    auto mat = getRandomMatrix(10, 20);
    ASSERT_NO_THROW(filterParallel(mat, 10));
}

TEST(Omp_Gaussian_blur, FilterParallel_Result_Equals_To_Filter_Result) {
    int width = 31;
    int coreSize = 5;
...
// TEST(Omp_Gaussian_blur, FilterParallel_Accelerates) {
