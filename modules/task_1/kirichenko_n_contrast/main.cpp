...
TEST(Image_contrast, Test1) {
    ...
    Result ñount = { 0, 254, 254, 0 };
    Result total = Contrast(manual);
    for (int i = 0; i < l * k; i++) {
        ASSERT_NEAR(total[i], ñount[i],1);
    }
}
TEST(Image_contrast, Test2) {
    ...
    Result ñount = { 0, 54, 90, 126, 162, 108, 144, 180, 252 };
    Result total = Contrast(manual);
    for (int i = 0; i < l * k; i++) {
        ASSERT_NEAR(total[i], ñount[i], 1);
    }
...
TEST(Image_contrast, Test3) {
    ...
    Result ñount = { 0, 1, 5, 14, 16, 12, 6, 9, 19, 25, 255, 230, 156, 78, 45, 96 };
    Result total = Contrast(manual);
    for (int i = 0; i < l * k; i++) {
        ASSERT_NEAR(total[i], ñount[i], 1);
    }
...
TEST(Image_contrast, Test4) {
    ...
    Result ñount = { 0, 26, 43, 25, 68, 29, 78, 95, 145, 156, 190, 201, 205, 220, 146, 250, 230, 35, 5, 26, 40, 18, 15, 75, 39, 255 };
    Result total = Contrast(manual);
    for (int i = 0; i < l * k; i++) {
        ASSERT_NEAR(total[i], ñount[i], 1);
    }
