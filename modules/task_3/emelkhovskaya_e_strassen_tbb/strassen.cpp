...
#include <string>
#include <stdexcept>
#include "../../modules/task_3/emelkhovskaya_e_strassen_tbb/strassen.h"
...
void StrassenFunc(double* a, double* b, double* c, int size) {
    ...
    tasks.run([=] { P1(a11, a22, b11, b22, prores1, size / 2); });
    tasks.run([=] { P2(a21, a22, b11, prores2, size / 2); });
    tasks.run([=] { P3(a11, b12, b22, prores3, size / 2); });
    tasks.run([=] { P4(a22, b21, b11, prores4, size / 2); });
    tasks.run([=] { P5(a11, a12, b22, prores5, size / 2); });
    tasks.run([=] { P6(a21, a11, b11, b12, prores6, size / 2); });
    tasks.run([=] { P7(a12, a22, b21, b22, prores7, size / 2); });
    tasks.wait();
