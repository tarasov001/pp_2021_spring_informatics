...
TEST(TBB_Dijkstra, DISABLED_Test_Time) {
    const size_t root = 0;
    const size_t vertexCount = 10000;
    const size_t edgeCount = vertexCount * (vertexCount - 1) / 2;
    tbb::tick_count timeStart, timeEnd;

    Graph graph(vertexCount, edgeCount);

    timeStart = tbb::tick_count::now();
    auto distancesSeq = algorithmDijkstraSeq(graph, root);
    timeEnd = tbb::tick_count::now();
    std::cout << "Sequential Time: " << (timeEnd - timeStart).seconds() << std::endl;

    timeStart = tbb::tick_count::now();
    auto distancesParallel = algorithmDijkstraParallel(graph, root);
    timeEnd = tbb::tick_count::now();
    std::cout << "Parallel Time: " << (timeEnd - timeStart).seconds() << std::endl;
}

TEST(TBB_Dijkstra, Test_DifferentVertexCount) {
...
TEST(TBB_Dijkstra, Test_IncorrectVertexCount) {
    ASSERT_ANY_THROW(Graph graph(0, 0));
}

TEST(TBB_Dijkstra, Test_GraphWithLoops) {
    const size_t vertexCount = 5;
...
TEST(TBB_Dijkstra, Test_CreateRandomGraph) {
    const size_t vertexCount = 5;
...
TEST(TBB_Dijkstra, Test_CreateRandomFullDefaultGraph) {
    ASSERT_NO_THROW(Graph graph);
}

TEST(TBB_Dijkstra, Test_DijkstraForHardcodeGraph) {
    const size_t vertexCount = 5;
    std::vector<size_t> eachVertex(vertexCount, 0);
    std::vector<std::vector<size_t>> distanceMatrix(vertexCount, eachVertex);
...
TEST(TBB_Dijkstra, Test_DijkstraForTwoHardcodeGraph) {
...
TEST(TBB_Dijkstra, Test_DijkstraSeqForRandomGraph) {
...
TEST(TBB_Dijkstra, Test_DijkstraForRandomGraph) {
...
TEST(TBB_Dijkstra, Test_DijkstraForRandomFullGraph) {
    const size_t vertexCount = 10;
    const size_t edgeCount = vertexCount * (vertexCount - 1) / 2;
